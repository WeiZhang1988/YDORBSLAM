...
namespace YDORBSLAM{
Viewer::Viewer(std::shared_ptr<System> _sptrSystem, std::shared_ptr<FrameDrawer> _sptrFrameDrawer, std::shared_ptr<MapDrawer> _sptrMapDrawer, std::shared_ptr<Tracking> _sptrTracking, const std::string& _strConfigurationPath):\
m_sptr_system(_sptrSystem), m_sptr_frameDrawer(_sptrFrameDrawer), m_sptr_mapDrawer(_sptrMapDrawer), m_sptr_tracker(_sptrTracking), m_b_finishRequested(false), m_b_finished(true), m_b_stopped(true), m_b_stopRequested(false){
    cv::FileStorage fileConfigs(_strConfigurationPath, cv::FileStorage::READ);
    if((float)fileConfigs["Camera.fps"]<1){
        m_db_period = 1e3/30;
    }else{
        m_db_period = 1e3/(float)fileConfigs["Camera.fps"];
    }
    m_flt_imageWidth = (float)fileConfigs["Camera.width"];
    m_flt_imageHeight = (float)fileConfigs["Camera.height"];
    if(m_flt_imageWidth<1 || m_flt_imageHeight<1)
    {
        m_flt_imageWidth = 640;
        m_flt_imageHeight = 480;
    }
    m_flt_viewpointX = (float)fileConfigs["Viewer.ViewpointX"];
    m_flt_viewpointY = (float)fileConfigs["Viewer.ViewpointY"];
    m_flt_viewpointZ = (float)fileConfigs["Viewer.ViewpointZ"];
    m_flt_viewpointF = (float)fileConfigs["Viewer.ViewpointF"];
}
void Viewer::run(){
    m_b_finished = false;
    m_b_stopped = false;
    ...
    pangolin::View& glDisplay = pangolin::CreateDisplay().SetBounds(0.0, 1.0, pangolin::Attach::Pix(175), 1.0, -1024.0f/768.0f)\
                                .SetHandler(new pangolin::Handler3D(glRenderObj));
