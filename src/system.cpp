...
namespace YDORBSLAM{
System::System(const std::string& _strVocFile, const std::string& _strConfigurationPath, const Sensor& _enumSensor, const bool& _bUseViewer):\
    ...
    if(m_enum_sensor == Sensor::STEREO){
        std::cout << "Stereo" << std::endl;
    }
    else if(m_enum_sensor == Sensor::RGBD){
    ...
    m_sptr_vocabulary = std::make_shared<DBoW3::Vocabulary>();
    ...
    m_sptr_tracker = std::make_shared<Tracking>(shared_from_this(),m_sptr_vocabulary,m_sptr_frameDrawer,m_sptr_mapDrawer,\
                                                m_sptr_map,m_sptr_keyFrameDatabase,m_enum_sensor,_strConfigurationPath);
    //Initialize the Local Mapping thread and launch
    m_sptr_localMapper = std::make_shared<LocalMapping>(m_sptr_map);
    m_sptr_threadLocalMapping = std::make_shared<std::thread>(&YDORBSLAM::LocalMapping::run,m_sptr_localMapper);
    //Initialize the Loop Closing thread and launch
    m_sptr_loopCloser = std::make_shared<LoopClosing>(m_sptr_map,m_sptr_keyFrameDatabase,m_sptr_vocabulary,false);
    m_sptr_threadLoopClosing = std::make_shared<std::thread>(&YDORBSLAM::LoopClosing::run,m_sptr_loopCloser);
    ...
    if(_bUseViewer){
        ...
        m_sptr_threadViewer = std::make_shared<std::thread>(&Viewer::run,m_sptr_viewer);
...
cv::Mat System::trackStereo(const cv::Mat& _cvMatLeftImage, const cv::Mat& _cvMatRightImage, const double& _dbTimeStamp){
    ...
    // Check mode change
    {
        ...
        if(m_b_activateLocalizationMode){
            ...
            m_sptr_tracker->informOnlyTracking(true);
            m_b_activateLocalizationMode = false;
        }
        else if(m_b_deactivateLocalizationMode){
            m_sptr_tracker->informOnlyTracking(false);
...
bool System::mapChanged(){
    ...
    int loopUpdateAndGBAtimes = m_sptr_map->getLastBigChangeIdx();
...
void System::shutdown(){
    ...
    while(!m_sptr_localMapper->isFinished() || !m_sptr_loopCloser->isFinished() || m_sptr_loopCloser->isRunningGlobalBA()){
        usleep(5000);
    }
    if(m_sptr_viewer){
...
void System::saveTrajectoryTUM(const std::string& _filename){
    ...
    trajectoryFile << std::fixed;
    // Frame pose is stored relative to its reference keyframe (which is optimized by BA and pose graph).
    // We need to get first the keyframe pose and then concatenate the relative transformation.
    // Frames not localized (tracking failure) are not saved.
    // For each frame we have a reference keyframe (lRit), the timestamp (lT) and a flag
    // which is true when tracking failed (lbF).
    std::list<std::shared_ptr<KeyFrame>>::iterator lRKFit = m_sptr_tracker->m_list_refKeyFrames.begin();
    std::list<double>::iterator lTit = m_sptr_tracker->m_list_frameTimes.begin();
    std::list<bool>::iterator lbFit = m_sptr_tracker->m_list_isLost.begin();
    for(cv::Mat& TcF_refKF : m_sptr_tracker->m_list_relFramePoses){
        if(*lbFit){
            continue;
        }
        ...
        trajectoryFile << std::setprecision(6) << *lTit << " " <<  std::setprecision(9) << tfirstKF_cF.at<float>(0) << " " << tfirstKF_cF.at<float>(1) \
...
void System::saveKeyFrameTrajectoryTUM(const std::string& _filename){
    ...
    std::ofstream KFTrajectoryFile;
    KFTrajectoryFile.open(_filename.c_str());
    KFTrajectoryFile << std::fixed;
    ...
    for(std::shared_ptr<KeyFrame>& sptrKF : vsptrAllKFs){
        ...
        if(!sptrKF->isBad()){
            ...
            KFTrajectoryFile << std::setprecision(6) << sptrKF->m_d_timeStamp << std::setprecision(7) << " " << Okf.at<float>(0) \
            << " " << Okf.at<float>(1) << " " << Okf.at<float>(2) << " " << q[0] << " " << q[1] << " " << q[2] << " " << q[3] << std::endl;
        }
    }
