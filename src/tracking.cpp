...
void Tracking::Track(){
    ...
    std::unique_lock<std::mutex> lock(m_sptr_map->m_mutex_map);
    if(m_enum_currentState == eTrackingState::NOT_INITIALIZED){
        ...
        if(btrackSuccess){
            ...
            if(!m_lastFrame.m_cvMat_T_c2w.empty()){
                cv::Mat cvMatLastTw2c = cv::Mat::eye(4,4,CV_32F);
                m_lastFrame.getRotation_w2c().copyTo(cvMatLastTw2c.rowRange(0,3).colRange(0,3));
                m_lastFrame.getCameraOriginInWorld().copyTo(cvMatLastTw2c.rowRange(0,3).col(3));
                m_cvMat_velocity = m_currentFrame.m_cvMat_T_c2w * cvMatLastTw2c;
            }else{
                m_cvMat_velocity = cv::Mat();
            }
            m_sptr_mapDrawer->setCurrentCameraPose(m_currentFrame.m_cvMat_T_c2w);
