...
class FrameDrawer{
    ...
    cv::Mat drawFrame();
    protected:
    void drawTextInfo(cv::Mat &_image, TrackingState _nState, cv::Mat &_imageText);
    // Info of the frame to be drawn
    cv::Mat m_cvMat_image;
    int m_int_CurrentKeyPointNUM;
    std::vector<cv::KeyPoint> m_v_cvMatCurrentKeyPoints;
    std::vector<bool> m_v_bOwnObservation, m_v_bNonObservation;
